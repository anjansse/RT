...
static void		cylinder_add(t_cylinder **cylinder, double base[3], double radius, double direction[3], double height, double rgb[3], double type)
{
	t_cylinder	*newCyl;
...
void            rt_store_cylinder(t_rt *rt, char *info)
{
	...
	store_number(infos[5], &type);
	cylinder_add(&cyl, base, radius, direction, height, rgb, type);
