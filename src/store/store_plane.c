...
static void		plane_add(t_plane **plane, double dist_to_origin, double xyz[3], double rgb[3], double type)
{
	t_plane		*new_plane;
...
void            rt_store_plane(t_rt *rt, char *info)
{
	...
	if (ft_array_len(infos) != 4)
		send_error(ft_strdup("Error in plane options -- should be [dist_to_origin] | [normal(x y z)] | [color(r g b)] | [type(type)].\n"));
	store_number(infos[0], &dist_to_origin);
	store_vector(infos[1], normal);
	store_vector(infos[2], rgb);
	store_number(infos[3], &type);
	plane_add(&plane, dist_to_origin, normal, rgb, type);
